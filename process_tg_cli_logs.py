...
presence_writer = csv.writer(online_presence_file, delimiter='\t', lineterminator='\n', quoting=csv.QUOTE_ALL)

message_count = 0
presence_count = 0

for logfile_path in sorted([f.path for f in os.scandir(tg_cli_logs_folder) if f.is_file() and f.name.startswith('complete_logs_')]):
    print(logfile_path)
